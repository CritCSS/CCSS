...
**Bias** can be introduced toying with the data. For example, I could have a race category in a dataset and group the variable into only White or Non-white, which would remove several populations from the analysis, leaving them unrepresented. Having domain-knowledge and sensibility about the data we are working on is key.
